...
var transformer = function() {
  ...
  function get(command, args, cc) {
    console.log("get() command=" + command + " args=" + args);
    var host = "us.api.battle.net";
    var path = "/wow/" + command + "/" + args + "?locale=en_US&apikey=b5wn89b7keavtn4zd9s4te6r9pggqruh";
    var options = {
      host: host,
      path: path,
      method: 'GET',
  ...
  function achievement(node, cc) {
    console.log("achievement()");
    var id = +visit(node.elts[0], cc);
    get("achievement", id, cc);
  }

  function data(node, cc) {
    console.log("data() node=" + JSON.stringify(node));
    var group = visit(node.elts[1]).tag;
    var kind = visit(node.elts[0]).tag;
    switch (group) {
    case "character":
    case "guild":
    case "item":
  ...
  function battlePet(node, cc) {
    var kind = visit(node.elts[1]).tag;
    var id = visit(node.elts[0]);
    var args = kind + "/" + id;
    get("battlePet", args, cc);
  }

  function challenge(node, cc) {
    console.log("challenge() node=" + JSON.stringify(node));
    var id = visit(node.elts[0]);
    var args;
    if (id.tag === "region") {
      args = "region";
...
exports.compiler = function () {
  ...
  function compile(src, cc) {
    transformer.transform(src, done);
    function done(data) {
      renderer.render(data, done);
      function done(data) {
        cc(data);
      }
    }
